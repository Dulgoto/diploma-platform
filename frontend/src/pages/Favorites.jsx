...
export default function Favorites() {
  ...
  const loadFavorites = useCallback(() => {
    ...
    get("/api/favorites")
      ...
      .catch(() => {
        setError("Неуспешно зареждане на любими.");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Любими</h1>
        <p className="text-sm text-slate-500">Обяви, които сте запазили.</p>
      </div>
      ...
      {!loading && !error && favorites.length === 0 ? (
        <p className="rounded-xl border border-slate-200 bg-white px-4 py-10 text-center text-sm text-slate-600 shadow-sm">
          Нямате любими обяви. Разгледайте{" "}
          <Link to="/ads" className="font-medium text-emerald-700 hover:underline">
            каталога
          </Link>
          .
        </p>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setFavorites((prev) => prev.filter((a) => a.id !== ad.id));
                        } catch (err) {
                          window.alert(errorMessage(err, "Неуспешно премахване от любими."));
                        } finally {
                          setRemovingId(null);
                        }
                      }}
                      className="rounded-lg border border-rose-200 bg-rose-50 px-2.5 py-1 text-xs font-medium text-rose-800 hover:bg-rose-100 disabled:opacity-60"
                    >
                      {removingId === ad.id ? "…" : "Премахни"}
                    </button>
                  </div>
                </div>
              </article>
